	/*
		...
		Used in `20180217a` experiment, to test alt.tbo v2
	...
	*/
	...
	#define REPEAT_SIZE 	20 // 10 captures	
	...
	*/
	...
	static void readScope(){
		...
		while(counterline<REPEAT_SIZE){ 
			...
			// testing With POn and Poff
			if ( counterline%2 == 0 ){
				//printk(KERN_ALERT "Even line on lune %d\n", counterline);
				GPIO_CLR = 1 << PPuls_ON;
				while(PonOne<10){ // Nb of NOPs for PulseOn 
					MY_NOP(__N); // 200ns
					PonOne++;
				}
				GPIO_SET = 1 << PPuls_ON;
				...
				//printk(KERN_ALERT "Odd line on lune %d\n", counterline);
				GPIO_CLR = 1 << NPuls_ON;
				while(PonTwo<10){ // Nb of NOPs for PulseOn 
					MY_NOP(__N); // 200ns
					PonTwo++;
				}
				GPIO_SET = 1 << NPuls_ON;
