...
![](https://raw.githubusercontent.com/kelu124/echomods/master/elmo/data/arduinoffset/LineImageEnveloppe.jpg)
...
* A [pulser board](https://kelu124.gitbooks.io/echomods/content/Chapter2/tobo.html) - to generate the high voltage pulse to use the transducer
* An [analog front end](https://kelu124.gitbooks.io/echomods/content/Chapter2/goblin.html) - to amplify the weak signals generated by the echoes
* an [ADC cape](https://kelu124.gitbooks.io/echomods/content/Chapter2/elmo.html) - a custom made 11Msps to 22Msps ADC pHAT for Raspberries.
* a [Raspberry Pi, 0 or W](https://kelu124.gitbooks.io/echomods/content/Chapter3/tomtom.html) to manage everything
* the [motherboard, v2](https://kelu124.gitbooks.io/echomods/content/Chapter3/doj.html) - to connect the different modules together
* and some extras such as headers, jumpers, and a smallish OLED screen

For the transducer, one can use either a [ATL mechanical probe](https://kelu124.gitbooks.io/echomods/content/Chapter2/retroATL3.html) for example, or a [piezo + servo setup](/cletus), to mechanically scan the medium to be imaged 
...
* __Disclaimer #0__: __This is not a medical ultrasound scanner__! It's a development kit that can be used for pedagogical and academic purposes - possible immediate use as a non-destructive testing ([NDT](https://kelu124.gitbooks.io/echomods/content/Chapter1/ndt.md)) tool, for example in metallurgical crack analysis. As in all electronics, be [careful](/WordOfCaution.md).
...
The first acquisition, used as a reference, is through the ADCs of a STM32F205 ([feather WICED](https://kelu124.gitbooks.io/echomods/content/Chapter3/croaker.html)) - reaching a whooping 1.7Msps (didn't master the interleaved mode..). Even at these speeds, one can see some features of an ultrasound image, on a wire-phantom. However, resolution isn't that great, let's see if that can be improved?

![](https://raw.githubusercontent.com/kelu124/echomods/master/croaker/data/20161217/20161217-222737-commented.png)
...
![](https://raw.githubusercontent.com/kelu124/echomods/master/elmo/data/arduino/setup.png)
...
* Simply the servo and transducer module ([cletus](https://kelu124.gitbooks.io/echomods/content/Chapter3/cletus.html)) -- get for _80$_ (Where? Recycling a transducer from ebay, servo from anywhere ([Amazon?](https://www.amazon.com/s/ref=nb_sb_noss?url=search-alias%3Dtoys-and-games&field-keywords=%22SG90+9G%22)))
* The Pi Heart of the echOmods ([tomtom](https://kelu124.gitbooks.io/echomods/content/Chapter3/tomtom.html)) -- get for _10$_ (Where?  Get the Pi W from [plenty of sources](https://www.raspberrypi.org/products/pi-zero-w/))
* Using a dual ADC raspberry extension for 20Msps+ DAQ ([elmo](https://kelu124.gitbooks.io/echomods/content/Chapter2/elmo.html)) -- get for _45$_ (Where? OSHPark, MacroFab)
* Goblin: a TGC-Envelop-ADC module ([goblin](https://kelu124.gitbooks.io/echomods/content/Chapter2/goblin.html)) -- get for _149$_ (Where? Custom made, get the [Gerbers](/goblin/source/), or [buy it preassembled on Tindie](https://www.tindie.com/products/kelu124/ultrasound-imaging-analog-processing-module/), or contact @kelu124)
* Tobo: the HV-pulser ([tobo](https://kelu124.gitbooks.io/echomods/content/Chapter2/tobo.html)) -- get for _120$_ (Where? Custom made, get the [Gerbers](/tobo/source/), [buy it preassembled on tindie](https://www.tindie.com/products/kelu124/ultrasound-imaging-pulser-module/) or contact @kelu124)
...
I'm getting similar images using the analog enveloppe detection (acquired by the [DAQ](https://kelu124.gitbooks.io/echomods/content/Chapter2/elmo.html), through a track exposed on the Analog Processing Module), compared to the digital enveloppe detection, as described below, again on the wire phantom:
...
![](https://raw.githubusercontent.com/kelu124/echomods/master/elmo/data/arduino/EnveloppeLineEnveloppe.jpg) 

### Plugging the Pi to an existing probe

A beaglebone black had been used with its [high-speed DAQ](/toadkiller/) to be connected to an existing [mechanical probe](https://kelu124.gitbooks.io/echomods/content/Chapter2/retroATL3.html), with [some results](https://kelu124.gitbooks.io/echomods/content/Chapter2/basicdevkit.html). The next step has been to interface the Pi to this probe through the [24Msps Pi ADC pHAT](https://kelu124.gitbooks.io/echomods/content/Chapter2/elmo.html), to see if one can get the same quality of image, and produce a ultrasound loop. 

It produced in the end actually good images:

![](https://raw.githubusercontent.com/kelu124/echomods/master/elmo/data/Imgs/pic_probeX.data.jpg)

along with details of the processing chain (see the [work jupyter notebook](/elmo/data/20170715-ProbeTest.ipynb)):

![](https://raw.githubusercontent.com/kelu124/echomods/master/elmo/data/Imgs/Processing_probeX.data.jpg)
