...
USBH_StatusTypeDef  USBH_Init(USBH_HandleTypeDef *phost, void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t ), uint8_t id)
{
  ...
  mico_rtos_init_queue( (mico_queue_t *) &(phost->os_event), NULL, sizeof(uint16_t), 10 );
  
  /*Create USB Host Task */
  mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "USBH", USBH_Process_OS, 0x4C0, (void*)phost );
...
#if (USBH_USE_OS == 1)
  mico_rtos_push_to_queue((mico_queue_t *) &(phost->os_event), &USBH_PORT, 0 );
#endif  
  return USBH_OK;  
}
...
#if (USBH_USE_OS == 1)
        mico_rtos_push_to_queue((mico_queue_t *) &(phost->os_event), &USBH_STATE_CHANGED, 0 );
#endif         
      }
    }
    break;
...
static void USBH_Process_OS(void * argument)
{
  ...
  for(;;)
  {
    mico_rtos_pop_from_queue( (mico_queue_t *)&(((USBH_HandleTypeDef *)argument)->os_event), &Receive, 0xFFFFFFFF );
      
      if(mico_rtos_is_queue_empty((mico_queue_t *)&(((USBH_HandleTypeDef *)argument)->os_event)))
      {
