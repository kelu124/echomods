...
class us_spi:
    ...
    JSON["parameters"] = {}
    JSON["timings"] = {}
    JSON["experiment"]["id"] = str(datetime.datetime.now().strftime("%Y%m%d"))+"b"
...
if __name__ == "__main__":
	...
	if (sys.argv[1] == "-f" ) and (len(sys.argv) == 3) and (os.path.exists(sys.argv[2])):
        ...
        if "loop" in sys.argv[1]:
            ...
            A = UN0RICK.set_timings(200, 100, 120, 2000, 5000, 200000)# Looping on triggers
            while True:
                UN0RICK.write_fpga(0xEA, 0x01)                   # trigs
                time.sleep(7.0 / 1000.0)                        # Waits 50ms between shots
