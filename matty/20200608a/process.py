...
class us_json:
    ...
    def create_fft(self):
        ...
        if self.processed:
            ...
            deb = int(500000/self.f/4)
            print(deb,sLT)
            plot_time = self.FFT_x[deb:sLT]
            plot_abs_fft = np.abs(self.FFT_y[deb:sLT])
            plot_filtered_fft = np.abs(self.filtered_fft[deb:sLT])
    ...
    def mkFiltered(self, original_image):
        ...
        plt.figure(figsize=(15, 10))
        #plt.imshow(np.sqrt(np.abs(  )))
        img_size=np.shape(self.filtered_image)
        plt.imshow(np.sqrt(np.abs(self.filtered_image)), cmap='gray', aspect=0.5*(img_size[1]/img_size[0]), interpolation='nearest')
            
        plt.title(self.create_title_text())
        plt.tight_layout()

        file_name = "images/2D_Filtered_"+self.iD+"-"+str(self.N)+".jpg"
        plt.savefig(file_name)
