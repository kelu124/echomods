...
class us_json:
    ...
    def mk2DArray(self):
        ...
        plt.figure(figsize = (15,5))
        im = plt.imshow(CleanImage, cmap='gray', aspect=0.3*(imSize[1]/imSize[0]), interpolation='nearest') 
        
        Title  = "Duration: "+str(Duration)+"us ("+str(self.parameters['LengthAcq'])+" - "
        Title += str(self.parameters['DeltaAcq'])+"), for "+str(self.Nacq)
        Title += " repeats\n"
        Title += "Fech = "+str(self.f)+"Msps, total of "+str(float(self.f)*Duration)+" pts per line\n" 
        Title += "Nacq = "+str(self.Nacq)+"\n"
        Title += "Timestamp = "+str(self.time)
    
        plt.title( Title  )
        #plt.colorbar(im, orientation='vertical')  
        plt.tight_layout()
        FileName = "images/2DArray_"+self.iD+".jpg"
        plt.savefig(FileName)
        plt.show() 
